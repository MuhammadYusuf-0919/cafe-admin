...
const Reports = () => {
  ...
  const topSellingItems = [
    { name: "Grilled Salmon", value: 32 },
    { name: "Steak", value: 27 },
    { name: "Coffee", value: 21 },
    { name: "Garlic Bread", value: 16 },
    { name: "Chocolate Cake", value: 11 },
  ];

  // Updated colors with teal palette
  const COLORS = ["#0FB5BA", "#0D989D", "#14B8D4", "#49BECD", "#2DD4BF"];
  ...
  return (
    <AppLayout title="Sales Reports">
      <motion.div
        ...
        className="mb-6"
      >
        <h1 className="text-2xl font-bold bg-gradient-to-r from-teal-600 to-teal-500 bg-clip-text text-transparent">Sales Reports</h1>
        <p className="text-gray-600 dark:text-gray-400">
          Analyze sales data and monitor restaurant performance
        </p>
      </motion.div>
            ...
            {date && (
              <div className="mt-4 space-y-2">
                <h3 className="font-medium">
                  Orders on {format(date, "MMMM d, yyyy")}
                </h3>
                <p className="text-gray-500 dark:text-gray-400">
                  {Math.floor(Math.random() * 10) + 5} orders
                </p>
                <p className="font-medium text-teal-600 dark:text-teal-500">
                  Total: ${((Math.random() * 300) + 200).toFixed(2)}
                </p>
              </div>
            )}
          </CardContent>
